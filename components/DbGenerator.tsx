...
3. 단원 매칭: PDF의 '유형 제목'을 키워드로 CSV 분류표에서 가장 유사한 행을 매칭한다. 
   **중요**: '과목', '대단원', '중단원', '소단원', '세부 유형'의 값은 반드시 CSV 목차 분류표에 있는 텍스트와 **토씨 하나 틀리지 않고 100% 동일하게** 출력해야 한다. 특히 수식(LaTeX 문법, 예: $y=\\log_a (px+q)+r$)이나 기호가 포함된 경우, 임의로 지우거나 수정하지 말고 CSV에 있는 그대로 출력하라.
